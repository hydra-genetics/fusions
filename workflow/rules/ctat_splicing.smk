...
rule ctat_splicing_call:
    ...
    shell:
        "(mkdir -p {params.temp_result_dir} && "
        ...
        "--ctat_genome_lib {params.ctat_genome_lib} || true; "
        "if [ ! -f {params.temp_result_dir}/{params.sample_name}.cancer.introns ]; then "
        "  echo -e 'intron\\tstrand\\tgenes\\tuniq_mapped\\tmulti_mapped\\tTCGA_sample_counts\\tGTEx_sample_counts\\tvariant_name' > {params.temp_result_dir}/{params.sample_name}.cancer.introns; "
        "fi; "
        "if [ ! -f {params.temp_result_dir}/{params.sample_name}.ctat-splicing.igv.html ]; then "
        "  echo '<html><body>No fusion reads detected for {params.sample_name}</body></html>' > {params.temp_result_dir}/{params.sample_name}.ctat-splicing.igv.html; "
        "fi; "
        "if [ ! -f {params.temp_result_dir}/{params.sample_name}.cancer_intron_reads.sorted.bam ]; then "
        "  samtools view -H {input.bam} | samtools view -bh - > {params.temp_result_dir}/{params.sample_name}.cancer_intron_reads.sorted.bam; "
        "  samtools index {params.temp_result_dir}/{params.sample_name}.cancer_intron_reads.sorted.bam; "
        "fi; "
